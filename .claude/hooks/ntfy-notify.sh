#!/bin/bash
# Hook: Send push notifications via ntfy.sh
...
# Load NTFY_TOPIC from shell config if not already in environment
if [ -z "${NTFY_TOPIC:-}" ]; then
  # Try zsh
  NTFY_TOPIC=$(sed -n 's/^export NTFY_TOPIC="\(.*\)"/\1/p' ~/.zshrc 2>/dev/null || true)
  # Try bash
  if [ -z "$NTFY_TOPIC" ]; then
    NTFY_TOPIC=$(sed -n 's/^export NTFY_TOPIC="\(.*\)"/\1/p' ~/.bashrc 2>/dev/null || true)
  fi
fi
if [ -z "$NTFY_TOPIC" ]; then
...
# Parse fields using python3 (available on macOS and most Linux)
