...
**⛔ NEVER use the generic `task` agent (Haiku) for code changes.** It lacks reasoning capacity for multi-file edits and often describes changes without executing them.
...
Directory-specific `CLAUDE.md` files are auto-loaded when working in those directories. Create them as needed:
- `tests/CLAUDE.md` — test conventions and fixtures
- `scripts/CLAUDE.md` — script-specific guidelines
...
|-------|---------|-------------|
| Sprint Planning | `/sprint-planning` | Triage backlog, score ICE, select sprint scope, move to Planned |
| Sprint Start | `/sprint-start` | Execute issues with quality gates and daily huddles |
| Sprint Review | `/sprint-review` | Demo deliverables, metrics, stakeholder acceptance |
| Sprint Retro | `/sprint-retro` | What went well/badly, process changes, velocity |
| Agent Memory | `/agent-memory` | Save, recall, organize persistent memories |
| Notify | `/notify` | Send push notifications via ntfy.sh |
| Code Review | `/code-review` | Structured review with checklists |
| Create PR | `/create-pr` | PRs with conventional commit titles |
| TDD Workflow | `/tdd-workflow` | Red-Green-Refactor cycle |
| Verification | `/verification-before-completion` | Evidence before claims |
| Writing Plans | `/writing-plans` | Implementation plans before code |
| Subagent Dev | `/subagent-driven-development` | Fresh subagent per task with review |
| GitHub Issues | `/github-issues` | Create, update, manage issues |
...
if [ -n "$NTFY_TOPIC" ]; then
  curl -s -H "Title: ✅ Task Complete" -d "Your message" ntfy.sh/$NTFY_TOPIC
fi
```

Priority levels: `urgent`, `high`, `default`, `low`, `min`

See `.claude/skills/notify/SKILL.md` for full documentation.
